...
namespace cs03_object
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            object iobj = (object)idata; // 박싱 : 데이터타입값을 object로 담기
            Console.WriteLine(iobj);
            Console.WriteLine(iobj.GetType());

            long udata = (long)iobj; // 언박싱 : object를 원래 데이터타입으로 바꾸기
            ...
            // 문자열을 숫자로
            // 문자열내에 숫자가 아닌 특수문자나 정수인데 .이 있으면 컴파일오류
            ...
            originstr = "123.4f";
            float ffval;
            // TryParse는 예외가 발생하면 값은 0으로 대체, 예외없으면 원래값으로 변환
