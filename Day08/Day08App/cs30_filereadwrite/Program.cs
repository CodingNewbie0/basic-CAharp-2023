...
namespace cs30_filereadwrite
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            try
            {
                reader = new StreamReader(@".\pyqt.py"); // 스트림리더 파일 연결
            ...
            {
                reader.Close(); // 파일 읽으면 무조건 마지막에 닫음
            }

            // 읽기 끝

            StreamWriter writer = new StreamWriter(@".\pyqtByCsharp.py");

            try
            {
                writer.WriteLine("import sys");
                writer.WriteLine("// 안녕");
                writer.WriteLine("print(sys.exctable)");
            }
            catch(Exception e)
            {
                Console.WriteLine($"예외! {e.Message}");
